...
class Ims_Portfolio_Public {
	...
	public function enqueue_styles() {
		 ...
		 * class.
		 */
		$filepath = 'css/ims-portfolio-public.css';
		wp_enqueue_style($this->plugin_name,plugin_dir_url(__FILE__) . $filepath,array(),filemtime(plugin_dir_path(__FILE__) . $filepath),'all');
	...
	public function enqueue_scripts() {
		...
		$jsfilepath = 'js/ims-portfolio-public.js';
	wp_enqueue_script($this->plugin_name,plugin_dir_url(__FILE__) . $jsfilepath,array('jquery'),filemtime(plugin_dir_path(__FILE__) . $jsfilepath),true
);
    ...
    wp_enqueue_script(
        'modernizr-script',
        $plugin_url . 'js/modernizr.custom.js',
        array(),
        filemtime( $plugin_dir . 'js/modernizr.custom.js' ), // version based on file modification time
        true
    );

    wp_enqueue_script(
        'classie-script',
        $plugin_url . 'js/classie.js',
        array(),
        filemtime( $plugin_dir . 'js/classie.js' ),
        true
    );

    wp_enqueue_script(
        'elastislide-script',
        $plugin_url . 'js/jquery.elastislide.js',
        array( 'jquery' ),
        filemtime( $plugin_dir . 'js/jquery.elastislide.js' ),
        true
    );

    wp_enqueue_script(
        'hoverdir-script',
        $plugin_url . 'js/jquery.hoverdir.js',
        array( 'jquery' ),
        filemtime( $plugin_dir . 'js/jquery.hoverdir.js' ),
        true
    );

    wp_enqueue_script(
        'elastic_grid-script',
        $plugin_url . 'js/elastic_grid.js',
        array( 'jquery' ),
        filemtime( $plugin_dir . 'js/elastic_grid.js' ),
        true
    );
