...
class Ims_Portfolio_Admin
{
	...
	public function enqueue_styles()
	{
		 ...
		 * class.
		 */
		$cssfilepath='css/ims-portfolio-admin.css';
		wp_enqueue_style($this->plugin_name,plugin_dir_url(__FILE__) .$cssfilepath,array(),filemtime(plugin_dir_path(__FILE__) . $cssfilepath),'all');
	}
	...
	public function enqueue_scripts()
	{
		 ...
		 * class.
		 */
	$filepath = 'js/ims-portfolio-admin.js';
	wp_enqueue_script($this->plugin_name,plugin_dir_url(__FILE__) . $filepath,array('jquery'),filemtime(plugin_dir_path(__FILE__) . $filepath),false);
	}
...
public function save_meta_boxes($post_id) {
    ...
    // Verify nonce
	if (! isset($_POST['ims_portfolio_nonce']) || ! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['ims_portfolio_nonce'])), 'save_ims_portfolio')) {
		return;
	}

    // Prevent during autosave
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
        return;
    }

    // Check user capabilities
    if (isset($_POST['post_type']) && 'ims_portfolio' === $_POST['post_type']) {
    ...
    $post_data = wp_unslash($_POST); // Unsanitize the array once

    if (isset($post_data['ims_button_name'])) {
        update_post_meta($post_id, 'ims_button_name', sanitize_text_field($post_data['ims_button_name']));
    }

    if (isset($post_data['ims_button_url'])) {
        update_post_meta($post_id, 'ims_button_url', esc_url_raw($post_data['ims_button_url']));
    }

    if (isset($post_data['ims_thumb_img'])) {
        update_post_meta($post_id, 'ims_thumb_img', sanitize_text_field($post_data['ims_thumb_img']));
    }

    if (isset($post_data['ims_large_img'])) {
        update_post_meta($post_id, 'ims_large_img', sanitize_text_field($post_data['ims_large_img']));
    }
}
	...
	public function admin_init()
	{
		...
		$labels = array(
			'name'               => __('Portfolio Tags', 'portfolio-gallery-by-ims'),
			'singular_name'      => __('Portfolio Tag', 'portfolio-gallery-by-ims'),
			'search_items'       => __('Search Portfolio Tags', 'portfolio-gallery-by-ims'),
			'all_items'          => __('All Portfolio Tags', 'portfolio-gallery-by-ims'),
			'parent_item'        => __('Parent Portfolio Tag', 'portfolio-gallery-by-ims'),
			'parent_item_colon'  => __('Parent Portfolio Tag:', 'portfolio-gallery-by-ims'),
			'edit_item'          => __('Edit Portfolio Tag', 'portfolio-gallery-by-ims'),
			'update_item'        => __('Update Portfolio Tag', 'portfolio-gallery-by-ims'),
			'add_new_item'       => __('Add New Portfolio Tag', 'portfolio-gallery-by-ims'),
			'new_item_name'      => __('New Portfolio Tag Name', 'portfolio-gallery-by-ims'),
			'menu_name'          => __('Portfolio Tags', 'portfolio-gallery-by-ims'),
		);
